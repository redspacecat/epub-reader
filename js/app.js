...
let sliderMobile = document.getElementById("font-size-slider-mobile")
sliderMobile.onchange = handleFontSizeMobile
...
function handleFontSizeMobile() {
    document.getElementById("content").style.fontSize = sliderMobile.value.toString() + "px"
}

function updateScroll() {
    c = document.getElementById("content")
    if (!(ereader.scrollingAllowed)) {
...
async function loadContent(page) {
    ...
    for (let i = 0; i < elements.length; i++) {
        if (elements[i].tagName == "A"/*.parentElement.parentElement.classList == "chapter"*/) {
            ...
            let id = searchManifestByHref((elements[i].attributes.href.value).replaceAll("../", ""))
            // if (id == undefined) {
...
function isScrollFinished() {
    ...
    const checkIfScrollToIsFinished = setInterval(() => {
        ...
        if (count > 30) {
            ...
            ereader.scrollEnd = c.scrollHeight
            ereader.scrollDifference = (ereader.scrollEnd - ereader.scrollStart) - c.offsetHeight
            // [BAD] ereader.scrollDifference = ereader.scroll - document.getElementById("content").scrollTop
            clearInterval(checkIfScrollToIsFinished);
        }
    }, 25);
}
...
if (ereader.isMobile) {
    ...
    if (ereader.scrollingAllowed) {
        document.getElementById("scroll-yes-normal").checked = true
        document.getElementById("scroll-no-normal").checked = false
    } else {
        document.getElementById("scroll-yes-normal").checked = false
        document.getElementById("scroll-no-normal").checked = true
    }
}
...
function updateSettingsVisible() {
    if (ereader.scrollingAllowed) {
        document.getElementById("scroll-yes").checked = true
        document.getElementById("scroll-no").checked = false
    } else {
        document.getElementById("scroll-yes").checked = false
        document.getElementById("scroll-no").checked = true
    }
}

function setScrollingAllowed(allowed) {
    ereader.scrollingAllowed = allowed
}